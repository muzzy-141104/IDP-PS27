...
def load_data_fidt(img_path, args, train=True):
    ...
    while True:
        try:
            ...
            k = np.asarray(gt_file['kpoint'], dtype=np.float32)  # Force float32 to save memory
            fidt_map = np.asarray(gt_file['fidt_map'], dtype=np.float32)  # Force float32 to save memory
            break
        except OSError:
