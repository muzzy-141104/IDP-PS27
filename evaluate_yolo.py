"""
Evaluate YOLO-Crowd model on ShanghaiTech Part A test set
...
def yolo_detect_count(model, img_path, device, img_size=640, conf_thres=0.3):
    """Run YOLO detection on a single image and return person count"""
...
def main():
    ...
    try:
        model, device = load_yolo_model(weights_path)
        print(f"Model loaded on {device}")
    except Exception as e:
        print(f"Error loading YOLO model: {e}")
        print("Trying alternative loading method...")
        # Fallback: load via torch.hub style
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        ...
        model.to(device)
        model.eval()
        print(f"Model loaded (fallback) on {device}")
    ...
    for i, img_file in enumerate(img_files):
        try:
            ...
            # Predict using YOLO
            pred_count = yolo_detect_count(model, img_path, device)
            ...
            if (i + 1) % 20 == 0:
                print(f"  [{i+1}/{len(img_files)}] {img_file} - GT: {gt_count}, Pred: {pred_count}")
    ...
    # ========== Save Metrics ==========
    with open(os.path.join(output_dir, 'yolo_metrics.txt'), 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("YOLO-CROWD MODEL EVALUATION RESULTS\n")
        f.write("=" * 60 + "\n\n")
        f.write(f"Model: YOLO-Crowd (Detection-based counting)\n")
        f.write(f"Dataset: ShanghaiTech Part A (Test Set)\n")
        f.write(f"Weights: yolo-crowd.pt\n\n")
    ...
    fig.suptitle('YOLO-Crowd Model Evaluation - ShanghaiTech Part A', fontsize=16, fontweight='bold')

    # 1. Scatter plot
    ax1 = axes[0, 0]
    ax1.scatter(y_true, y_pred, alpha=0.5, s=25, c='darkorange', edgecolors='saddlebrown', linewidths=0.5)
    ...
    # 2. Error histogram
    ax2 = axes[0, 1]
    ax2.hist(errors, bins=40, edgecolor='black', alpha=0.7, color='darkorange')
    ...
    width = 0.35
    ax3.bar(x - width / 2, y_true[:n_samples], width, label='Ground Truth', alpha=0.8, color='darkorange')
    ax3.bar(x + width / 2, y_pred[:n_samples], width, label='Predicted', alpha=0.8, color='steelblue')
    ...
    bars = ax4.bar(bin_labels, bin_mae, color='darkorange', edgecolor='black')
    ...
    chart_path = os.path.join(output_dir, 'yolo_evaluation.png')
    plt.savefig(chart_path, dpi=150, bbox_inches='tight')
    plt.close()

    # Print results
    print("\n" + "=" * 60)
    print("YOLO-CROWD EVALUATION RESULTS")
    print("=" * 60)
    print(f"MAE:  {mae:.2f}")
    ...
    print(f"Metrics saved to: {os.path.join(output_dir, 'yolo_metrics.txt')}")
